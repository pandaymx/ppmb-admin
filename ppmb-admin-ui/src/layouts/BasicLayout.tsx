...
const BasicLayout: React.FC = () => {
  ...
  const generateMenuItems = (routeList: AppRouteObject[], basePath = ""): MenuItem[] => {
    ...
    routeList.forEach((route) => {
      ...
      const currentPath = route.path?.startsWith("/") ? route.path : `${basePath}/${route.path}`.replace(/\/+/g, "/");
      ...
      const validChildren = route.children?.filter(
        (child) => !(child as AppRouteObject).meta?.hideInMenu && !child.index && hasPermission(child as AppRouteObject)
      ...
      if (route.meta?.title) {
        items.push({
          ...
          children: hasChildren ? generateMenuItems(validChildren, currentPath) : undefined,
  ...
  const rootRoute = routes.find((r) => r.path === "/") as AppRouteObject | undefined;
  const menuItems = rootRoute?.children ? generateMenuItems(rootRoute.children, "/") : [];
