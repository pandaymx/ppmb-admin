for file in ppmb-common-redis/src/test/java/top/ppmblszdp/common/redis/util/*Test.java; do
    ...
    /public static GenericContainer<\?> redisContainer = new GenericContainer<>\(DockerImageName.parse/ {
        print "    @Container"
        print "    public static GenericContainer<?> redisContainer = new GenericContainer<>(DockerImageName.parse(\"public.ecr.aws/docker/library/redis:7-alpine\").asCompatibleSubstituteFor(\"redis\"))"
