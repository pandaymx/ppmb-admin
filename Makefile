.PHONY: infra backend frontend app all stop clean setup setup-root setup-frontend setup-docs docs help
...
help:
	...
	@echo "  make docs      - 启动文档服务 (Docusaurus)"
	@echo "  make app       - 启动应用 (后端 + 前端，不启动基础设施；适合 worktree)"
	@echo "  make all       - 启动全部 (基础设施 + 后端 + 前端)"
	@echo "  make setup     - 安装依赖 (根目录工具链 + 前端依赖 + 文档依赖)"
	@echo "  make stop      - 停止基础设施"

# 安装依赖（面向新同学/首次运行）
setup: setup-root setup-frontend setup-docs
...
setup-docs:
	@if [ -d ppmb-docs ]; then \
		if [ ! -d ppmb-docs/node_modules ]; then \
			echo "安装文档依赖 (ppmb-docs, bun install)..."; \
			cd ppmb-docs && bun install; \
		fi; \
	fi
