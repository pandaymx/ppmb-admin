...
public class AuditEntityListener implements PostInsertEventListener, PostUpdateEventListener, PostDeleteEventListener {
  ...
  public void onPostInsert(PostInsertEvent event) {
    if (event.getEntity().getClass().isAnnotationPresent(Auditable.class)) {
      publishEvent("INSERT", event.getEntity(), event.getId(), null, event.getState(), event.getPersister());
    }
  }
  ...
  public void onPostUpdate(PostUpdateEvent event) {
    if (event.getEntity().getClass().isAnnotationPresent(Auditable.class)) {
      publishEvent("UPDATE", event.getEntity(), event.getId(), event.getOldState(), event.getState(), event.getPersister());
    }
  }
  ...
  public void onPostDelete(PostDeleteEvent event) {
    if (event.getEntity().getClass().isAnnotationPresent(Auditable.class)) {
      publishEvent("DELETE", event.getEntity(), event.getId(), event.getDeletedState(), null, event.getPersister());
    }
  }
  ...
  private void publishEvent(String operation, Object entity, Object id, Object[] oldState, Object[] newState, EntityPersister persister) {
    try {
        ...
        if (oldState != null) {
            ...
            for (int i = 0; i < propertyNames.length; i++) {
                // Avoid serializing associations that might cause LazyInitializationException
