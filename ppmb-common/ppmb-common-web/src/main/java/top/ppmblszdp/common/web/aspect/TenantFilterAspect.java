...
/**
 * Aspect to automatically enable Hibernate filter for tenant isolation on Spring Data JPA Repositories.
 */
@Aspect
@Component
  ...
  public Object enableTenantFilter(ProceedingJoinPoint pjp) throws Throwable {
    ...
    if (tenantId != null) {
      ...
      if (repositoryInterface != null) {
        Class<?>[] typeArguments = GenericTypeResolver.resolveTypeArguments(repositoryInterface, org.springframework.data.repository.Repository.class);
        if (typeArguments != null && typeArguments.length > 0) {
          Class<?> entityClass = typeArguments[0];
          // If it's not a global table, enable the filter
          if (entityClass != null && !entityClass.isAnnotationPresent(GlobalTable.class)) {
