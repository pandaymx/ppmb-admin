...
public interface MqMessageOutboxRepository extends JpaRepository<MqMessageOutbox, Long> {
  ...
  /**
   * Find messages that need to be retried, including those with null nextRetryTime. Use Pageable to
   * limit the result size and avoid OOM.
   *
   ...
   * @return List of messages to retry.
   */
  @Query(
      "SELECT m FROM MqMessageOutbox m WHERE m.status IN :statuses "
