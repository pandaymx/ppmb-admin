...
public class ReliableMessageServiceImpl implements ReliableMessageService {
  ...
  public void send(String exchange, String routingKey, CommonMessage<?> message) {
    ...
    // I also updated the query to handle NULLs, but let's initialize it properly as well for the
    // first try.
        ...
        new TransactionSynchronization() {
          ...
          public void afterCommit() {
            try {
              ...
              outboxRepository.save(outbox);
              log.debug(
                  "Successfully published reliable message for outbox id: {}", outbox.getId());
            } catch (Exception _) {
              // On failure, do nothing in this transaction (since it's afterCommit, the original
              // business
              // transaction is already successful). Keep it as PENDING and let the retry job handle
              // it.
              log.warn(
                  "Failed to publish message after commit for outbox id: {}. Retrying later.",
                  outbox.getId());
            }
          }
        });
